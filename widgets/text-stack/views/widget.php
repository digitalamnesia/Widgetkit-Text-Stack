<?php
// Plug-in modified by nicola.raddino@pursang.graphics
// If you have some upgrade please report to yootheme, github -> link or send me an email!
// Enjoy.

// DEFINE MEDIA Width
$media_width = '{wk}-width-' . $settings['breakpoint'] . '-' . $settings['width'];
// DEFINE SECOND CONTENT WIDTH
$text_width = '{wk}-width-' . $settings['breakpoint'] . '-' . $settings['text_width'];
// CONTENT WIDTH DEFINE THE OTHER CONTENT WIDTH
switch ( $settings['text_width']) {
    case '1-5':
        $content_width = '4-5';
        break;
    case '1-4':
...
// DEFINE MAIN CONTENT WIDTH ON THE PREVIOUS PROPORTION
...
<?php foreach ($items as $i => $item) :  ?>
    ...
    <?php
        ...
        if (($item->type('media') == 'image') && ($settings['image_width'] != 'auto' || $settings['image_height'] != 'auto')) {
			...
			$second_media = $item->media('second_media', $attrs);
        }
        ...
        // MEDIA & SECOND MEDIA LINK AND OVERLAY
        if ($settings['media_overlay'] == 'link' || $settings['media_overlay'] == 'icon' || $settings['media_overlay'] == 'image') {
			...
			$second_media = '<div class="{wk}-overlay {wk}-overlay-hover ' . $border . '">' . $second_media;
            ...
            if ($settings['media_overlay'] == 'icon') {
				...
				$second_media .= '<div class="{wk}-overlay-panel {wk}-overlay-background {wk}-overlay-icon {wk}-overlay-' . $settings['overlay_animation'] . '"></div>';
            }

            if ($item['link']) {
                $media .= '<a class="{wk}-position-cover" href="' . $item->escape('link') . '"' . $link_target . '></a>';
				$second_media .= '<a class="{wk}-position-cover" href="' . $item->escape('second_link') . '"' . $link_target . '></a>';
            }
        ...
        if ($socials && $settings['media_overlay'] == 'social-buttons') {
			...
			#SECOND
			$second_media  = '<div class="{wk}-overlay {wk}-overlay-hover ' . $border . '">' . $second_media;
            $second_media .= '<div class="{wk}-overlay-panel {wk}-overlay-background {wk}-overlay-' . $settings['overlay_animation'] . ' {wk}-flex {wk}-flex-center {wk}-flex-middle {wk}-text-center"><div>';
            $second_media .= '<div class="{wk}-grid {wk}-grid-small" data-{wk}-grid-margin>' . $socials . '</div>';
            $second_media .= '</div></div>';
            $second_media .= '</div>';
        }

        // Align
        if ($settings['alternate']) {
            $align_flip = $i % 2 == ($settings['second_align'] == 'left');
        } else {
            $align_flip = ($settings['second_align'] == 'right');
        }
        ...
        // CONTENT WIDTH WITHOUT ANY MEDIA
    ...
    <div class="<?php echo $grid; ?> {wk}-text-<?php echo $text_align; ?> <?php echo $content_align; ?>" data-{wk}-grid-match data-{wk}-margin="{cls:'{wk}-margin-top'}">
        ...
        <div class="<?php echo $text_width; ?><?php if ($align_flip) echo ' {wk}-float-right {wk}-flex-order-last-' . $settings['breakpoint']; ?><?php if ($settings['animation_content'] != 'none') echo ' {wk}-invisible'; ?>" <?php echo $animation_content; ?>>
            <div class="<?php echo $panel; ?> {wk}-width-1-1">
                ...
                <?php #SECOND TITLE
				if ($item['second_title'] && $settings['second_title']) : ?>
                <h3 class="<?php echo $title_size; ?>">

                    <?php if ($item['second_link']) : ?>
                        <a class="{wk}-link-reset" href="<?php echo $item->escape('second_link'); ?>"<?php echo $link_target; ?>><?php echo $item['second_title']; ?></a>
                    <?php else : ?>
                ...
                <?php #SECOND CONTENT
				if ($item['second_content'] && $settings['second_content']) : ?>
                <div class="{wk}-margin"><?php echo $item['second_content']; ?></div>
                <?php endif; ?>
                ...
                <?php if ($item['second_link'] && $settings['second_link']) : ?>
                <p><a<?php if($link_style) echo ' class="' . $link_style . '"'; ?> href="<?php echo $item->escape('second_link'); ?>"<?php echo $link_target; ?>><?php echo $app['translator']->trans($settings['link_text']); ?></a></p>
                <?php endif; ?>		

            </div><!-- CLOSING PANEL 1-1 -->
        </div><!-- CLOSING FIRST GRID CHILD -->
        <?php endif; ?>
        ...
        <div class="<?php echo $item_content_width; ?><?php if ($settings['animation_content'] != 'none') echo ' {wk}-invisible'; ?>" <?php echo $animation_content; ?>>
            <div class="<?php echo $panel; ?> {wk}-width-1-1">
                ...
                <?php 
				#MAIN CONTENT
				if ($item['content'] && $settings['content']) : ?>
                <div class="{wk}-margin"><?php echo $item['content']; ?></div>
                <?php endif; ?>
				
				<?php
				# ADDING FIRST MEDIA IF EXIST
				if ($item['media'] && $settings['media']) : ?>
				<div class="<?php echo $item_media_width; ?> {wk}-text-center<?php if ($settings['animation_media'] != 'none') echo ' {wk}-invisible'; ?>" <?php echo $animation_media; ?>>
				<?php echo $media; ?>
				</div>
